...
$t_subproject_ids = project_hierarchy_get_subprojects( $f_project_id, TRUE );
...
if( 0 == $f_user_id ) {
    ...
    foreach( $t_subproject_ids as $count_project_id ) {
        project_remove_all_users( $count_project_id, access_get_project_level( $count_project_id ) );
    }
} else {
    ...
    helper_ensure_confirmed( lang_get( 'remove_user_sure_msg' ) .
            '<br />' . lang_get( 'username_label' ) . lang_get( 'word_separator' ) . $t_user['username'], lang_get( 'remove_user_button' ) );

    project_remove_user( $f_project_id, $f_user_id );
    foreach( $t_subproject_ids as $count_project_id ) {
        project_remove_user( $count_project_id, $f_user_id );
    }
}
