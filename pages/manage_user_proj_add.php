...
foreach( $f_project_id as $t_proj_id ) {
    $t_subproject_ids = project_hierarchy_get_all_subprojects( $t_proj_id, TRUE );
    ...
    foreach( $t_subproject_ids as $t_subject_id ) {
        if( access_has_project_level( $t_manage_user_threshold, $t_subject_id ) &&
                access_has_project_level( $f_access_level, $t_subject_id ) && !array_key_exists(  $f_user_id, project_get_all_user_rows( $t_subject_id )  )) {
            
            project_add_user( $t_subject_id, $f_user_id, $f_access_level );
        }
    }
}
