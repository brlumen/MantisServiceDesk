...
# along with MantisBT.  If not, see <http://www.gnu.org/licenses/>.
	...
	// Добавляем каждый user_id в отслеживаемые
	foreach ($_POST['user_ids'] as $multi_user_id) {

		$multi_user_id = trim(htmlspecialchars($multi_user_id));
		$t_logged_in_user_id = auth_get_current_user_id();

		if ( is_blank( $multi_user_id ) ) {
			$t_user_id = $t_logged_in_user_id;
		} else {
			$t_user_id = $multi_user_id;
