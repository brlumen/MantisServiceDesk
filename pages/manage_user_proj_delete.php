...
helper_ensure_confirmed( lang_get( 'remove_user_sure_msg' ) .
        '<br />' . lang_get( 'project_name_label' ) . lang_get( 'word_separator' ) . $t_project_name, lang_get( 'remove_user_button' ) );

$t_subproject_ids = project_hierarchy_get_all_subprojects( $f_project_id, TRUE );

project_remove_user( $f_project_id, $f_user_id );
foreach( $t_subproject_ids as $t_subject_id ) {
    project_remove_user( $t_subject_id, $f_user_id );
}
