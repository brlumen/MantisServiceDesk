...
//                    break;
	...
	function check_issue( $type_event, $p_updated_bug, $p_existing_bug ) {
		...
		//check handler is not null before issue update
		if( $p_updated_bug->handler_id == NULL ) {
			error_parameters( lang_get( 'assigned_to' ) );
			trigger_error( ERROR_EMPTY_FIELD, ERROR );
		}

		//check comment is not null before issue update
		if( plugin_config_get( 'check_comments' ) ) {
			$f_bug_note_text = gpc_get_string( "bugnote_text", '' );
			$f_status	 = gpc_get_int( 'status', NULL );

			$t_bugnote_text = trim( $f_bug_note_text );

			if(
				is_blank( $t_bugnote_text ) &&
				$f_status != 90 &&
				$f_status != null
			) {
